...
const TRACKS = [
  { value: 500, labelKey: 'sprint', hint: '500m' },
  { value: DEFAULT_TRACK_METRES, labelKey: 'standard', hint: '1000m' },
  { value: 2000, labelKey: 'marathon', hint: '2000m' },
] as const;
    ...
    <div className="flex flex-col gap-6">
      <fieldset>
        <legend className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-ink-faint">
          {t('host:brainRace.trackLength')}
        </legend>
        <div
          role="radiogroup"
          aria-label={t('host:brainRace.trackLength')}
        ...
        >
          {TRACKS.map((option) => {
            ...
            return (
              ...
              >
                {t(`host:brainRace.${option.labelKey}`)}
                <span
      ...
      <fieldset>
        <legend className="flex w-full items-baseline justify-between">
          <span className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-ink-faint">
            {t('host:brainRace.finishersToWin')}
          </span>
          <span className="text-[11px] font-semibold text-ink-faint">
            {t('host:brainRace.perTeam')}
          </span>
        </legend>
        <p className="mt-1 text-xs font-semibold text-ink-faint">{t('host:brainRace.autoHint')}</p>
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          >
            {t('host:brainRace.auto')}
          </button>
        </div>
        {!autoFinishers ? (
          <div className="mt-3 flex items-center gap-2">
            <FinishersStepButton
              label={t('host:brainRace.fewerFinishers')}
              onClick={() => onFinishersChange(Math.max(1, (finishersRequiredPerTeam ?? 1) - 1))}
            >
              &minus;
            </FinishersStepButton>
            <output className="tabular flex-1 rounded-card border-2 border-paper-line bg-paper-card py-2.5 text-center font-display text-lg font-extrabold text-ink">
              ...
              <span className="text-xs font-bold text-ink-faint">
                {t('common:units.finishers')}
              </span>
            </output>
            <FinishersStepButton
              label={t('host:brainRace.moreFinishers')}
              onClick={() =>
                onFinishersChange(Math.min(FINISHERS_MAX, (finishersRequiredPerTeam ?? 1) + 1))
              }
            >
              +
            </FinishersStepButton>
