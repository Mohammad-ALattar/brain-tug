 ...
 */
export function useTugOfWarMotion(ref: RefObject<HTMLElement | null>): void {
  useEffect(() => {
    ...
    const apply = (store: GameStore): void => {
      const mode = store.state?.modeState;
      const rope = mode?.kind === 'tug_of_war' ? clampRope(mode.ropePosition) : 0;
      const rules = store.state?.rules ?? null;
      const tension = rules && mode?.kind === 'tug_of_war' ? ropeTension(rules, rope) : 0;
      ...
      if (flash && flash.key !== lastPullKey) {
        lastPullKey = flash.key;
        brace(flash.teamId);
      }
    };
