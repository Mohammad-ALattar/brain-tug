...
export const PullingBanner = memo(function PullingBanner() {
  const ropePosition = useRopePosition();
  ...
  const metres = Math.abs(ropeToMetres(rules, ropePosition));
  const scoreAdvantage = Math.abs(blueScore - redScore);
  const tone =
    leader === 'blue'
      ? 'bg-blueteam-600 text-white'
      : 'bg-redteam-600 text-white';
